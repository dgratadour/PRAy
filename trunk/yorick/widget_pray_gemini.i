...
func pray_set_starpos(type,nstars)
{
  if (type == 0) {
    ...
    for (cc=1;cc<=nstars;cc++) {
      pyk,swrite(format=cmd_pray+"glade.get_widget('starx%d').set_text('%s')",cc,swrite(format="%f",pos(2*cc-1)));
      pyk,swrite(format=cmd_pray+"glade.get_widget('stary%d').set_text('%s')",cc,swrite(format="%f",pos(2*cc)));
    }
  }
  ...
  if (type == 1) {
    ...
    */
    ...
    for (cc=1;cc<=numberof(aposx);cc++) {
      pyk,swrite(format=cmd_pray+"glade.get_widget('starx%d').set_text('%s')",cc,swrite(format="%f",aposx(cc)));
      pyk,swrite(format=cmd_pray+"glade.get_widget('stary%d').set_text('%s')",cc,swrite(format="%f",aposy(cc)));
    }
    pyk,swrite(format=cmd_pray+"glade.get_widget('spin_stars').set_value(%f)",19.);
...
func pray_file_load(filename,images=) {
  ...
  if (dims(2) != dims(3)) {
    pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')",  "WARNING : Wrong image size. data should be square arrays.");
    return;
  }

  if (dims(1) == 3) {
    //we are dealing with classical phase diversity data
    pyk,swrite(format=cmd_pray+"glade.get_widget('spin_stars').set_value(%d)",1);
    pyk,swrite(format=cmd_pray+"glade.get_widget('spin_layers').set_value(%d)",1);
    ...
    pyk,swrite(format=cmd_pray+"glade.get_widget('spin_defocpos').set_value(%d)",pray_ndefoc);
    ...
    for (cc=1;cc<=100;cc++)
      pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').remove_text(%d)",0);
    
    pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').set_sensitive(%d)",1);
    pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').insert_text(%d,'%s')",0,"Foc");
    for (cc=1;cc<=pray_ndefoc;cc++) {
      pname = swrite(format="deFoc %d",cc);
      pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').insert_text(%d,'%s')",cc,pname);
    }
    
    pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').set_active(%d)",0);
    pyk,swrite(format=cmd_pray+"glade.get_widget('combo_modetype').set_active(%d)",0);
    pyk,swrite(format=cmd_pray+"glade.get_widget('combo_objtype').set_active(%d)",1);
    pyk,swrite(format=cmd_pray+"glade.get_widget('getcoeffs').set_sensitive(%d)",1);

    pyk,swrite(format=cmd_pray+"glade.get_widget('textview2').get_buffer().set_text('%s')",
               "You have just loaded a 3-dimensional array.");
    pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","So I will run in classical mode.");
    ...
    else pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')",
    ...
    if (dims(1) == 4) {
      ...
      for (cc=1;cc<=100;cc++)
        pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_target').remove_text(%d)",0);
      
      //pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').set_sensitive(%d)",1);
      //pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_target').set_sensitive(%d)",1);
      ...
      for (cc=1;cc<=ntarget;cc++) {
        ...
        pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_target').insert_text(%d,'%s')",cc,pname);
      }
      
      pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_disp').set_active(%d)",0);
      pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_pray_target').set_active(%d)",0);
      ...
      pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","So I will run in 3D mode.");
      ...
      pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Npix x Npix x Ntarget x (Ndefoc+1) for 3D phase diversity");
      pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Npix x Npix x (Ndefoc+1) for classical phase diversity");
      pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Please load another file");
    }
  }    
  pray_setcuts;
}

func start_pray(nstars,targetx,targety,nlayers,alts,nmodes,boxsize,ndefoc,deltaFoc_nm,lambda_im,teldiam,cobs,pix_size,obj_type,obj_size,modetype,nbiter,disp,thresh,scalar,useguess,scale,diff_tt,fit_starpos,fit_object)
{
  extern pray_buffer,pray_buffer_data;
  
  pyk,swrite(format=cmd_pray+"glade.get_widget('textview2').get_buffer().set_text('%s')",
             "Starting PRAy on the data you provided ...");
  pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","You can abort anytime by pressing the abort button");
  pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Please be cool and press only once ;-)");
  ...
  if (anyof(deltaFoc == 0)) {
    pyk,swrite(format=cmd_pray+"glade.get_widget('textview2').get_buffer().set_text('%s')",
               "---------------------------------------------------------------");
    pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Wrong defoc value entered");
    if (pray_ndefoc >= 2)
      pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')",
                 "Please enter the defoc values in the boxes below");
    else pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')",
  ...
  pray_update_zernike_table,array(0.,nzer(1)),lambda_im,nzer(1);
  ...
  for (cc=1;cc<=10;cc++)
    pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_error_disp').remove_text(%d)",0);

  pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_error_disp').set_sensitive(%d)",1);
  pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_error_disp').insert_text(%d,'%s')",0,"Error on Foc");
  for (cc=1;cc<=pray_ndefoc;cc++) {
    pname = swrite(format="Error on deFoc %d",cc);
    pyk,swrite(format=cmd_pray+"glade.get_widget('winselect_error_disp').insert_text(%d,'%s')",cc,pname);
  }
  ...
  if (scalar) res = pray(images,xpos,ypos,deltaFoc,sqrt(var),object,lambda_im,nzer=nzer,alt=alt,
                         ...
                         fit_starpos=fit_starpos,fit_object=fit_object);
  else {
...
func pray_create(nstars,targetx,targety,nlayers,alts,nmodes,ndefoc,deltaFoc_nm,lambda_im,teldiam,cobs,pix_size,obj_type,obj_size,modetype,size,snr,diff_tt,fit_starpos,fit_object)
{
  ...
  pray_update_zernike_table,array(0.,100),lambda_im,nzer(1);
  ...
  // create psfs
  modes_coeff = [gaussdev(nzer(1)-1)*0.5/(indgen(nzer(1)-1))](*);
  // this are the mode coefficients
  if (nlayers > 1)
    for (i=2;i<=nlayers;i++) modes_coeff=_(modes_coeff,[gaussdev(nzer(i))*0.5/(indgen(nzer(i)))](*));
  ...
  if (diff_tt) {
    diff_coeff = gaussdev(2*ndefoc)*0.5; //2 * ndefoc coeffs
  ...
  pray_update_zernike_table,coeff,lambda_im,nzer(1);
  pyk,swrite(format=cmd_pray+"y_add_comment_txt_nl('%s')","Using the modes coefficients and defoc values printed below");
...
{
  pyk,swrite(format=cmd_pray+"glade.get_widget('progressbar_pray').set_fraction(%f)",float(frac));  
}

func pray_progressbar_text(text)
{
  pyk,swrite(format=cmd_pray+"glade.get_widget('progressbar_pray').set_text('%s')",text);  
}

func pray_dispgui_update
{
  //pyk,swrite(format="y_pray_set_lut(%d)",pray_lut);
  pyk,swrite(format=cmd_pray+"pray_set_itt(%d)",clip(long(pray_itt-1),0,4));
...
func pray_disp_cpc(e,all=)
{
  ...
  pray_cmin = cmin;
  pray_cmax = cmax;
  pyk,swrite(format=cmd_pray+"pray_set_cmincmax(%f,%f,%f)",float(cmin),float(cmax),float(cmax-cmin)/100.);
}
...
if (anyof(strmatch(arg_pray,"widget_pray_gemini.i")) || get_env("EMACS")=="t" ) {
  cmd_pray = "";
  python_exec = praytop+"/widgets/widget_pray_gemini.py";
  pyk_cmd=[python_exec];
  if (!_pyk_proc) _pyk_proc = spawn(pyk_cmd, _pyk_callback);
  write,"widget_pray_gemini  ready";
